...
\chapter{Introduction}
    ...
    As previously stated, a mobile robot is free to move in its environment, which immediately begs the question of how to describe such a system. A complete specification of the robot's points is called \textit{configuration} of the robot, which will be assumed as the interconnection of fully rigid links by means of joints. This simplification greatly reduces the number of parameters needed to describe the robot's full configuration, as is known by any course in mechanics; then, the minimum number of real-valued coordinates needed to represent the configuration is called \textit{Degrees Of Freedom} of the system. By merging the two definitions, the \textit{Configuration Space} of a robot can be defined as:
    \begin{definition}
    ...
    \begin{definition}
        ...
        \end{equation}
        The equation is exact in the case of fully independent constraints.
    \end{definition}
